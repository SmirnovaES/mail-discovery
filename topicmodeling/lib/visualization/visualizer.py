...
class Visualizer:
    def __init__(self, docs_words, topics_words, id2word, topics_terms):
    ...
    def run(self, path, metrics='tfIdf'):
        ...
        for i in range(topics_num):
            ...
            names = np.array(list(map(lambda x: x[0], self.topics_terms[i])))
            probs = np.array(list(map(lambda x: x[1], self.topics_terms[i])))

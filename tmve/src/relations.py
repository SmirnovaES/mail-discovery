...
class relations:
    ...
    def cmp_topics(self, a, b):
        a = self.get_overall_score(a)
        b = self.get_overall_score(b)
        return -cmp(a, b)
    ...
    def get_topics(self):
        ...
        self.topics.sort(key=functools.cmp_to_key(lambda x, y: -cmp(self.get_overall_score(x), self.get_overall_score(y))))
    ...
    def get_terms(self):
        if self.terms == []:
            ...
            self.terms.sort(key=functools.cmp_to_key(lambda x, y: -cmp(self.get_term_count(x), self.get_term_count(y))))
    ...
    def get_related_docs(self, token):
        ...
        if isinstance(token, Topic):
            ...
            token_doc_info = self.mydb.get_doc_docs(
                token.id)  # TODO: id vs doc_id: make docs, topics, etc more consistent
    ...
    def get_related_topics(self, token):
        ...
        for info in token_topic_info:
            score = info[3]
            if score != 0 and not (
                    isinstance(token, Document) and score < 1):  # check for reverse pairs in topic-topic search

...
def get_doc_display(doc):
    ...
    end_cur = site.find('</h1>'.encode())

    start_cur = site[end_cur:].find('<table class="vertical-navbox nowraplinks"'.encode())
    while (start_cur != -1):
        while (start_cur != -1):
    ...
    new_start_cur = site[end_cur:].find('<p'.encode())
    new_end_cur = site[end_cur:].find('</p>'.encode())

    pair_key = 0
    pairs = [('<p', '</p>'), ('<span class="mw-headline"', '</span>'), ('<div id="toctitle">', '</div>'),
             ('<ul>', '</ul>')]
    ...
    while (new_start_cur != -1 and new_end_cur != -1 and len(content) < 5000):
        ...
        new_end_cur = -1
        for pair in pairs:
            start_cur = site[end_cur:].find(pair[0].encode())
            if start_cur != -1 and (new_start_cur == -1 or start_cur < new_start_cur):
                new_start_cur = start_cur
                new_end_cur = site[end_cur:].find(pair[1].encode())
                temp_start_cur = new_start_cur
                count = 0
                while count < 10 and site[end_cur + temp_start_cur + len(pair[0]): end_cur + new_end_cur].find(
                        pair[0].encode()) != -1:
                    temp_start_cur = temp_start_cur + site[end_cur + temp_start_cur + len(pair[0]):].find(
                        pair[0].encode()) + len(pair[0])
                    new_end_cur = new_end_cur + site[end_cur + new_end_cur + len(pair[1]):].find(pair[1].encode()) + len(pair[1])
                    count += 1
        ...
        for i in range(num_terms):
            ...
            for top in topic_set:
                if top == '' or top.get_term(i) == None:
                    ...
                    topic_table += '<td onclick="window.location.href=\'../terms/' + top.get_term(
                        i).title + '.html\'">' + top.get_term(i).title + '</td>\n'
            topic_table += '</tr>'
...
def get_xml_topic_graph(relations):
    ...
    for topic in topics:
        ...
        term_keys.sort(key=functools.cmp_to_key(lambda x, y: -cmp(terms[x], terms[y])))
        ...
        for term in term_keys:
            ...
            if (per != 0):
                terms_table += '<td class="clickable" width="' + str(
                    per * 100) + '%" title="' + term.title + '" onclick="window.location.href=\'../terms/' + term.get_safe_title() + '.html\'"></td>\n'
                total_percent += per
            else:
                if remaining_terms == '':
        ...
        if (100 - total_percent) > 0:
            terms_table += '<td width="' + str(
                (1 - total_percent) * 100) + '%" title="' + remaining_terms + '">&nbsp;</td>\n'
...
def get_xml_topic_presence_graph(relations):
    ...
    for topic in topics:
        ...
        topic_table += '<tr><td><table width="' + str(width) + '%" ><tr><td class="high-contrast" title="' + str(int(
            overall_topic_score)) + '"onclick="window.location.href=\'../topics/' + topic.get_safe_title() + '.html\'">' + topic.title + '</td></tr></table></td></tr>\n'
...
def get_xml_term_graph(relations):
    ...
    for term in terms:
        ...
        topic_keys.sort(key=functools.cmp_to_key(lambda x, y: cmp(topics[x], topics[y])))
        ...
        for topic in topic_keys:
            ...
            if (per != 0):
                topics_table += '<td class="clickable" width="' + str(
                    per * 100) + '%" title="' + topic.title + '" onclick="window.location.href=\'../topics/' + topic.get_safe_title() + '.html\'"></td>\n'
                total_percent += per
            else:
                if remaining_topics == '':
        ...
        if remaining_topics != '' and (100 - total_percent) > 0:
            topics_table += '<td width="' + str(
                (1 - total_percent) * 100) + '%" title="' + remaining_topics + '">&nbsp;</td>\n'
    ...
    for i in range(0, len(terms), 8):
        ...
        for j in range(8):
            if (i + j) < len(terms):
                ...
                terms_table += '<td title="' + str(int(term_count)) + '" onclick="window.location.href=\'../terms/' + \
                               terms[i + j].get_safe_title() + '.html\'" onmouseover="count_adjust(' + str(
                    100.0 * (term_count - relations.term_score_range[0]) / (
                                relations.term_score_range[1] - relations.term_score_range[0]) + 1) + ')">' + terms[
                                   i + j].title + '</td>\n'
            else:
...
def get_xml_topic_docs_column(relation, topic):
    ...
    doc_keys.sort(key=functools.cmp_to_key(lambda x, y: -cmp(docs[x], docs[y])))
...
def get_xml_doc_topics_column(relation, doc):  # TODO: some of these functions can be collapsed/generalized
    ...
    for top in topic_keys:
        topic_column += '<tr class="list"><td id="' + top.get_safe_title() + '" onmouseover="highlight(' + str(
            topic_num) + ')" onmouseout="unhighlight()" onclick="window.location.href=\'../topics/' + top.get_safe_title() + '.html\'">' + top.title + '</td></tr>\n'
        topic_num += 1
...
def get_js_doc_topic_pie_array(relations, doc):
    ...
    for key in topic_keys:
        array_string += "[" + str(
            topics[key]) + ", " + "\"../topics/" + key.get_safe_title() + ".html\", \"" + key.get_safe_title() + "\"]"
        key_count += 1
...
def get_js_topic_term_pie_array(relations, topic):
    ...
    for term in terms:
        ...
        array_string += "[" + str(
            rel_percent) + ", " + "\"../topics/" + term.get_safe_title() + ".html\", \"" + term.get_safe_title() + "\"]"
        term_count += 1
